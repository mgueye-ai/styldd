...
Deno.serve(async (req) => {
  ...
  if (mode === "embedded") {
    ...
    form.set(
      "description",
      `Your brand name deposit (${styleLabel})${isHouse ? " + house-call deposit" : ""} · ${bookingId.slice(0, 8)}…`,
    );
  ...
  form.set("line_items[0][price_data][product_data][name]", `Your brand name - deposit (${styleLabel})`);
  const depDesc =
