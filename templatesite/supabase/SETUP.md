# Supabase setup (Your Salon Name)
...
2. **Domain** (recommended for production): **Domains** → **Add domain** → enter your site domain (e.g. `yoursalon.com`). Add the **DNS records** Resend shows (often DKIM + SPF) at your DNS host (Cloudflare, GoDaddy, etc.). Wait until the domain shows **Verified**.
3. **API key**: **API Keys** → create a key with permission to send email. Copy it once and store it in a password manager.
4. **From address**: After the domain is verified, you can send from something like `Your Salon Name <bookings@yourdomain.com>`. That exact string is what you put in `RESEND_FROM` below.
...
1. **Secrets** — Dashboard → **Project Settings** → **Edge Functions** → **Secrets** (or CLI below). Add:
   ...
   | `RESEND_FROM` | `Your Salon Name <bookings@yourdomain.com>` — must be allowed by Resend |
   | `NOTIFY_TO_EMAIL` | Where salon notifications go (e.g. the owner Gmail or `hello@yourdomain.com`) |
   | `SALON_SITE_NOTIFY_SECRET` | Long random string (e.g. `openssl rand -hex 32`). **Anyone with this secret can trigger notifications** — keep it only in Supabase + webhook config. |
   | `PUBLIC_SITE_URL` | Optional. Your live site base URL with **no** trailing slash (e.g. `https://yoursalon.com`). Adds a **View your booking details** link in the customer confirmation email. `SITE_URL` is accepted as an alias. |

   CLI equivalent:

   ```bash
   npx supabase secrets set RESEND_API_KEY=re_xxx RESEND_FROM="Your Salon Name <bookings@yourdomain.com>" NOTIFY_TO_EMAIL=you@gmail.com SALON_SITE_NOTIFY_SECRET="$(openssl rand -hex 32)"
   ```
...
4. **Database Webhooks** (optional — salon email already sends from the site via `notify-salon`):
   ...
   - **Create** two hooks (or one hook with a filter if your plan supports it):
     ...
     **Hook A — `bookings` INSERT**
     ...
     - If using HTTP POST, set header **`x-salon-site-notify-secret`** to the **same** value as `SALON_SITE_NOTIFY_SECRET`.

     **Hook B — `inquiries` INSERT**

     - Table: `public.inquiries`
     - Events: **Insert**
     - Same function URL and **`x-salon-site-notify-secret`** header as Hook A.
