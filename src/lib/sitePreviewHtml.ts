import { formatSiteAddress, SiteContent } from '../data/siteContent';
...
export function buildSitePreviewInjection(
  ...
  return `window.__STYLD_SITE_CONTENT__=${JSON.stringify(content)};window.__STYLD_SITE_STYLES__=${JSON.stringify(styles)};window.__STYLD_SITE_THEME__=${JSON.stringify(theme)};`;
}
...
<div class="catalog-grid">${buildStyleCardsHtml(styles)}</div></div></section>
...
${content.email ? `<p>${escapeHtml(content.email)}</p>` : ''}
