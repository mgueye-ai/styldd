...
/** Pick readable text on a surface, honoring site ink when it already contrasts. */
...
export function resolveStyldEmailBranding(input: {
  ...
  // Mirror tenant-site.js: light ink without a saved background implies a dark site.
  let pageBg = theme.backgroundColor || DEFAULT_CREAM;
  ...
  const headerBg = theme.navbarColor || primaryColor;
