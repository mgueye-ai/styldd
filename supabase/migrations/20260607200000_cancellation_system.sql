-- Cancellation policy, audit log, and cancel context RPC.

create table if not exists public.styld_cancellation_events (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users (id) on delete cascade,
  booking_id uuid not null,
  cancelled_by text not null check (cancelled_by in ('client', 'stylist')),
  refund_eligible boolean not null default false,
  refund_amount_cents integer not null default 0,
  refund_status text not null default 'none' check (
    refund_status in ('none', 'pending', 'succeeded', 'failed', 'skipped')
  ),
...
create index if not exists styld_cancellation_events_user_id_idx
  on public.styld_cancellation_events (user_id, created_at desc);

create index if not exists styld_cancellation_events_booking_id_idx
  on public.styld_cancellation_events (booking_id);

alter table public.styld_cancellation_events enable row level security;

drop policy if exists "Users read own cancellation events"
  on public.styld_cancellation_events;

create policy "Users read own cancellation events"
  on public.styld_cancellation_events
  for select
  to authenticated
  using (auth.uid() = user_id);

grant select on public.styld_cancellation_events to authenticated;
grant all on public.styld_cancellation_events to service_role;

-- Context for manage-booking page (no cancel side effects).
