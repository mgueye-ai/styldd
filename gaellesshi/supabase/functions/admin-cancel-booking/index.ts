...
import { sendCustomerBookingCancelledSms, sendSalonBookingCancelledSms } from "../_shared/salon-sms.ts";
...
Deno.serve(async (req) => {
  ...
  const salonSms = await sendSalonBookingCancelledSms(row);
  const custSms = await sendCustomerBookingCancelledSms(row);
  if (!salonSms.ok && !salonSms.skipped) {
    console.warn("Salon cancel SMS failed:", salonSms.error);
  }
  if (!custSms.ok && !custSms.skipped) {
    console.warn("Customer cancel SMS failed:", custSms.error);
  }
